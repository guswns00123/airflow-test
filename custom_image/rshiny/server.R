server <- function(input, output){
	selected_data <- reactive({
		...
		#Get the data
		corona <- dbGetQuery(conn, glue("SELECT to_date(substring(\"S_DT\",1,10),'YYYY.MM.DD') as s_dt, \"N_HJ\"::float as n_hj, \"T_HJ\" as t_hj FROM \"TbCorona19CountStatus_bulk2\" WHERE to_date(substring(\"S_DT\",1,10), 'YYYY.MM.DD') BETWEEN '{format(input$dates[1])}' AND '{format(input$dates[2])}'"))
